...
struct Road01
{
    ...
    /// コンストラクタ
    /// </summary>
    public Road01(Vector2 pos1, Vector2 pos2, Vector2 lanes)
    {
        this.pos1 = pos1;
        this.pos2 = pos2;
        this.lanes = lanes;
...
/// </summary>
[ExecuteAlways]
public class RoadPlane01 : MonoBehaviour
{
    ...
    {
        var count = 3;
        roadsBuffer = new ComputeBuffer(count, Marshal.SizeOf(typeof(Road01)));

        // 配列に初期値を代入する
        Road01[] roads = new Road01[count];
        var step = MAP_SIZE / (count+1);
        var x = step;
        for (int i = 0; i < count; i++)
        {
            roads[i] = new Road01(new Vector2(x, 0.1f * MAP_SIZE) , new Vector2(x, 0.9f * MAP_SIZE), new Vector2(2,2));
            x += step;
        }
